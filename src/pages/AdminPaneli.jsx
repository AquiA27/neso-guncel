...
function AdminPaneli() {
  ...
  async function verileriGetir() {
    try {
      ...
      setOrders(Array.isArray(sipRes.orders) ? sipRes.orders.reverse() : []);
      setGunluk(gunRes || { siparis_sayisi: 0, gelir: 0 });
      setAylik(ayRes || { siparis_sayisi: 0, gelir: 0 });
      setYillik(
        yilRes && typeof yilRes === "object"
          ? Object.entries(yilRes).map(([tarih, adet]) => ({ tarih, adet }))
          : []
      );
      setPopuler(Array.isArray(popRes) ? popRes : []);
      setOnline(onRes.count ?? 0);
      setMenu(Array.isArray(menuRes.menu) ? menuRes.menu : []);
    } catch (err) {
